(function($){
    ...
    function animate_scroll(){
		...
		$('.scroll-left-to-right').each(function() {
			var left = $(window).scrollTop();
			var e_top = $(this).offset().top;
			var e_height = $(this).outerHeight();
			var max = 800; 
			var start = e_top - max - $(window).height();                 // Tinh vi tri bat dau hieu ung
			var stop = e_top + e_height + max - $(window).height();       // Tinh vi tri ket thuc hieu ung
			
			if (left > start && left < stop) {
				var x = (left - start - max) / 10;
				if (x < -100) x = -100;
				if (x > 100) x = 100;
		...
		$('.scroll-right-to-left').each(function() {
			var left = $(window).scrollTop();
			var e_top = $(this).offset().top;
			var e_height = $(this).outerHeight();
			var max = 800;
		...
		// $('.scroll-sm-to-lg').each(function() {
		// 	var sm = $(window).scrollTop();
		// 	var e_top = $(this).offset().top;
		// 	var e_size = $(this).outer
		// })

		$(window).on('scroll', function() {
			$('.scroll-small-to-big').each(function() {
				var top = $(window).scrollTop();
				var e_top = $(this).offset().top;
				var e_height = $(this).height();
				var max = 1000;  // Adjust this value as needed
				var start = e_top - max - $(window).height();
				var stop = e_top + e_height + max - $(window).height();
				if (top > start && top < stop) {
					var scale = 1 + ((top - start - max) / (stop - start)) * 0.5;  // Adjust the 0.5 to control the maximum scale
					if (scale < 1) scale = 1;
					if (scale > 1.5) scale = 1.5;  // Adjust the 1.5 to control the maximum scale limit
		...
		$(window).on('scroll', function() {
			$('.scroll-big-to-small').each(function() {
				var top = $(window).scrollTop();
				var e_top = $(this).offset().top;
				var e_height = $(this).height();
				var max = 1000;  // Adjust this value as needed
				...
				var stop = e_top + e_height + max - $(window).height();
				if (top > start && top < stop) {
					var scale = 1.2 - ((top - start - max) / (stop - start)) * 0.3;  // Adjust the 0.3 to control the maximum scale reduction
					if (scale < 0.8) scale = 0.8;  // Ensure the element doesn't shrink below its original size
					if (scale > 1.2) scale = 1.2;  // Set the initial scale to be the maximum
					$(this).css('transform', 'scale(' + scale + ')');
				}
			});
		});
		...
		$(window).on('scroll', function() {
			$('.scroll-rotate-left-to-right').each(function() {
				var top = $(window).scrollTop();
				var e_top = $(this).offset().top;
				var e_height = $(this).height();
				var max = 1000;  // Điều chỉnh giá trị này nếu cần
				var start = e_top - max - $(window).height();
				var stop = e_top + e_height + max - $(window).height();
				if (top > start && top < stop) {
					var rotation;
					if (top > $(this).data('lastTop')) {
						// Cuộn xuống: xoay từ trái sang phải
						rotation = ((top - start) / (stop - start)) * -90;
					} else {
						// Cuộn lên: xoay từ phải sang trái
						rotation = ((stop - top) / (stop - start)) * -90;
					}
					$(this).css('transform', 'rotate(' + rotation + 'deg)');
				}
				$(this).data('lastTop', top);
			});
		});
		...
		$(window).on('scroll', function() {
			$('.scroll-rotate-right-to-left').each(function() {
				var top = $(window).scrollTop();
				var e_top = $(this).offset().top;
				var e_height = $(this).height();
				var max = 1000;  // Điều chỉnh giá trị này nếu cần
				...
				if (top > start && top < stop) {
					var rotation;
					if (top > $(this).data('lastTop')) {
						// Cuộn xuống: xoay từ phải sang trái
						rotation = ((top - start) / (stop - start)) * 90;
					} else {
						// Cuộn lên: xoay từ trái sang phải
						rotation = ((stop - top) / (stop - start)) * 90;
					}
					$(this).css('transform', 'rotate(' + rotation + 'deg)');
				}
	...
	function detail_gallery(){
		...
		if($('.detail-gallery').length>0){
			...
			$('input[name="variation_id"]').on('change',function(){
				...
				var srcfull =  $('.detail-gallery').find('img').attr("data-src");
				$('.zoomWindow,.zoomLens').css('background-image','url("'+srcfull+'")');
				$.removeData($('.detail-gallery .mid img'), 'elevateZoom');//remove zoom instance from image
				$.removeData($('.flex-viewport img'), 'elevateZoom');
				$('.zoomContainer').remove();
				$('.detail-gallery').find('.zoom-style1 .mid img').data('zoom-image', srcfull).elevateZoom();
				$('.detail-gallery').find('.zoom-style2 .mid img').data('zoom-image', srcfull).elevateZoom({
					scrollZoom : true
				});
				$('.detail-gallery').find('.zoom-style3 .mid img').data('zoom-image', srcfull).elevateZoom({
					zoomType: "lens",
					lensShape: "square",
					lensSize: 150,
					borderSize:1,
				...
				$('.detail-gallery').find('.zoom-style4 .mid img').data('zoom-image', srcfull).elevateZoom({
					zoomType: "inner",
					cursor: "crosshair",
					zoomWindowFadeIn: 500,
					zoomWindowFadeOut: 750
