...
if((isset($check_share[$post_type]) && $check_share[$post_type] == '1') || $check_share_page == '1'):
	$list_default = array(
		array(
			'title'  => esc_html__('Total',"autopart"),
		    'social' => 'total',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Facebook',"autopart"),
		    'social' => 'facebook',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Twitter',"autopart"),
		    'social' => 'twitter',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Google',"autopart"),
		    'social' => 'google',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Pinterest',"autopart"),
		    'social' => 'pinterest',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Linkedin',"autopart"),
		    'social' => 'linkedin',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Tumblr',"autopart"),
		    'social' => 'tumblr',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Email',"autopart"),
		    'social' => 'envelope',
		    'number' => '1',
			),
		);
		...
		foreach ($list as $value) {
			switch ($value['social']) {
				...
				case 'envelope':
					...
					echo 	'<li><a target="_blank" data-social="'.esc_attr($value['social']).'" title="'.esc_attr($value['title']).'" href="mailto:?subject='.esc_attr__("I wanted you to see this site&amp;body=Check out this site","autopart").' '.get_the_permalink().'">
								<span class="share-icon '.esc_attr($value['social']).'-social"><i class="la la-'.esc_attr($value['social']).'" aria-hidden="true"></i>'.$number_html.'</span>
							</a></li>';
					break;
