...
if(class_exists("woocommerce")){
    if(!function_exists('th_filter_price')){
        function th_filter_price($min,$max,$filtered_posts = array()){
            global $wpdb;
    ...
    add_action( 'woocommerce_after_single_product_summary', 'th_add_to_cart_sticky', 35 );
    ...
    if(!function_exists('th_woocommerce_thumbnail_loop')){
        function th_woocommerce_thumbnail_loop($size,$animation = '',$echo = true) {
            ...
            if($animation == 'rotate-thumb' || $animation == 'zoomout-thumb' || $animation == 'translate-thumb') {
                ...
                if(isset($img_hover['id']) && !empty($img_hover['id'])) $img_hover_html = wp_get_attachment_image($img_hover['id'],$size);
                else{
                    global $product;
                    $attachment_ids = $product->get_gallery_image_ids();
                    if(isset($attachment_ids[0]) && !empty($attachment_ids[0])) $hover_id = $attachment_ids[0];
                    else $hover_id = get_post_thumbnail_id();
                    $img_hover_html = wp_get_attachment_image($hover_id,$size);
                }
            }
    ...
    if(!function_exists('th_product_quickview')){
        function th_product_quickview($icon = '<i class="la la-search"></i>',$class = '',$echo = true) {
            $html = '<a title="'.esc_attr__("Quick View","autopart").'" data-product-id="'.get_the_id().'" href="'.esc_url(get_the_permalink()).'" class="product-quick-view quickview-link '.esc_attr($class).'">'.$icon.'</a>';
    ...
    if(!function_exists('th_product_label')){
        function th_product_label($echo = true) {
            ...
            if($uppsell < 0) $html .=   '<span class="new">'.esc_html__("new","autopart").'</span>';
        ...
        function th_get_rating_html($echo = true, $count = true, $style = ''){
            ...
            switch ($style) {
                case 'style2':
                    ...
                    if($count){
                        $html .=    '<li>
                                        <span class="number-rate silver">('.$review_count.' '.esc_html__("Rating","autopart").')</span>
                                    </li>';
                                        }
                    $html .=    '   <li>
    ...
    if ( !function_exists( 'th_catalog_ordering' ) ) {
        function th_catalog_ordering($query,$set_orderby = '',$list_item = false,$add_class = '') {        
            ...
            $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
                'menu_order' => esc_html__( 'Default sorting', 'autopart' ),
                'popularity' => esc_html__( 'Sort by popularity', 'autopart' ),
                'rating'     => esc_html__( 'Sort by average rating', 'autopart' ),
                'date'       => esc_html__( 'Sort by newness', 'autopart' ),
                'price'      => esc_html__( 'Sort by price: low to high', 'autopart' ),
                'price-desc' => esc_html__( 'Sort by price: high to low', 'autopart' )
            ) );
            ...
            if ( wc_get_loop_prop( 'is_search' ) ) {
                $catalog_orderby_options = array_merge( array( 'relevance' => esc_html__( 'Relevance', 'autopart' ) ), $catalog_orderby_options );
    ...
    if(!function_exists('th_load_product_filter')){
        function th_load_product_filter() {
            ...
            $load_data = str_replace('\/', '/', $load_data);
            $load_data = json_decode($load_data,true);  
            if($load_data === NULL) $load_data = $data_obj;
            // var_dump($load_data);
            // $load_data = $data_obj;     
            if($load_data){
                extract($load_data);
                if(isset($attr['item_wrap'])) $attr['item_wrap'] = str_replace('\"', '"', $attr['item_wrap']);
                if(isset($attr['item_inner'])) $attr['item_inner'] = str_replace('\"', '"', $attr['item_inner']);
                extract($attr);            
                ...
                <div class="products row list-product-wrap js-content-main">
                    ...
                    else echo '<div class="filter-noresult-wrap"><div class="filter-noresult title18 text-center">'.esc_html__("No result found with current filter value.","autopart").'</div></div>';                        
                        ?>
                </div>
                <?php
    ...
    if(!function_exists('th_add_to_cart_sticky')){
        function th_add_to_cart_sticky(){            
            $check = th_get_value_by_id('add_to_cart_sticky');
            if($check == 'show'){
                ?>
                <div class="add-to-cart-sticky">
                    <div class="container">
                        <div class="sticky-inner">
                            <div class="sticky-thumb"><?php the_post_thumbnail(array(100,100))?></div>
                            <div class="sticky-name"><?php the_title()?></div>
                            <div class="sticky-button"><?php woocommerce_template_single_add_to_cart()?></div>
                        </div>
                    </div>
                </div>
                <?php
    ...
    if(!function_exists('th_get_cat_in')){
        function th_get_cat_in($cat='',$taxonomy = 'product_cat',$style = ''){
            $html = '';
            $terms = wp_get_post_terms( get_the_id(), $taxonomy, array('parent '=> ''));
            $term_name = $term_link = $term_html = '';
            if(empty($cat)){
                if(is_array($terms) && !empty($terms)){
                    $term_name = $terms[0]->name;
                    $term_count = $terms[0]->count;
            ...
            if($term_name && $term_link){
                switch ($style) {
                    case 'style2':
                        $html = '<a href="'.esc_url($term_link).'" class="cat-parent">'.$term_name.'</a>';
                        break;
...
if(!function_exists('th_live_search')){
    function th_live_search() {
        ...
        if( $query->have_posts() && !empty($key) && !empty($trim_key)){
            ...
            echo '<p class="text-center">'.esc_html__("No any results with this keyword.","autopart").'</p>';
        }
        wp_reset_postdata();
    }
}