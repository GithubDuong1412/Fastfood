...
if(!class_exists('Th_BaseController')){
    class Th_BaseController{
        static function _init(){
            ...
            $prd_taxonomies = array('product_make','product_model','product_year','product_trim');
            foreach ($prd_taxonomies as $tax_name){
                add_action('tax_'.$tax_name.'_add_form_fields', array(__CLASS__,'th_product_taxonomy_metabox_add'), 20, 1);
                add_action('tax_'.$tax_name.'_edit_form_fields', array(__CLASS__,'th_product_taxonomy_metabox_edit'), 100, 1);
                add_action('created_'.'tax_'.$tax_name, array(__CLASS__,'th_product_save_taxonomy_metadata'), 20, 1);
                add_action('edited_'.'tax_'.$tax_name, array(__CLASS__,'th_product_save_taxonomy_metadata'), 20, 1);
            }
        ...
        static function _blog_filter_sidebar($sidebar){
            ...
            if(class_exists( 'WooCommerce' )){
                if(th_is_woocommerce_page()){
                    ...
                    $sidebar_style = th_get_option('woo_sidebar_style');
                    if(is_single()){
                        $pos = th_get_option('sv_sidebar_position_woo_single');
                        $sidebar_id = th_get_option('sv_sidebar_woo_single');
                        $sidebar_style=th_get_option('woo_single_sidebar_style');
                    }
                }
            }
            if(is_archive() && !th_is_woocommerce_page()){
                ...
                $sidebar_style=th_get_option('archive_sidebar_style');
            }
            else{
                if(!is_home() && !is_single()){
                    ...
                    $single_sidebar_style= get_post_meta('single_sidebar_style');
                    if(!empty($single_sidebar_style)){
                        $sidebar_style = $single_sidebar_style;
                    }
            ...
            if(is_search()) {
                ...
                if($post_type != 'product'){
                    ...
                    $sidebar_style=th_get_option('search_sidebar_style');
                }              
            }
                ...
                foreach($add_sidebars as $sidebar){
                    ...
                    if(!empty($sidebar['title'])){
                        ...
                        $custom_add_sidebar = array(
                                'name' => $sidebar['title'],
                                'id' => $id,
                                'description' => esc_html__( 'SideBar created by add sidebar in theme options.', 'autopart'),
        ...
        static function th_product_cat_metabox_add($tag) { 
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content Before','autopart'); ?></label>
                <div class="wrap-metabox">
                    <select name="before_append" id="before_append">
                        <option value=""><?php esc_html_e("Choose page","autopart")?></option>
                        <?php
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content After','autopart'); ?></label>
                <div class="wrap-metabox">
        ...
        static function th_product_taxonomy_metabox_add($tag){
            /*
            Product taxonomy:
            tax_product_make,
            tax_product_model,
            ...
            */
            switch ($tag){
                default:
                    break;
                case ('tax_product_make'):
                    break;
                case ('tax_product_model'):
                    ...
                    ?>
                    <div class="form-field">
                        <label class="font-bold"><?php esc_html_e('Related Make','autopart'); ?></label>
                        <div class="wrap-metabox">
                            <?php
                            foreach ($make_terms as $key => $make_term){
                                ?>
                                <input type="checkbox" name="related_make_tax[]" value="<?php echo esc_attr($make_term->slug) ?>" /> <span><?php echo esc_attr($make_term->name) ?></span>
                                <?php
                            }
                            ?>
                        </div>
                        <label class="font-bold"><?php esc_html_e('Related Year','autopart'); ?></label>
                        <div class="wrap-metabox">
                            <?php
                            foreach ($year_terms as $key => $year_term){
                                ?>
                                <input type="checkbox" name="related_year_tax[]" value="<?php echo esc_attr($year_term->slug) ?>" /> <span><?php echo esc_attr($year_term->name) ?></span>
                                <?php
                            }
                            ?>
                        </div>
                        <label class="font-bold"><?php esc_html_e('Related Trim','autopart'); ?></label>
                        <div class="wrap-metabox">
                            <?php
                            foreach ($trim_terms as $key => $trim_term){
                                ?>
                                <input type="checkbox" name="related_trim_tax[]" value="<?php echo esc_attr($trim_term->slug) ?>" /> <span><?php echo esc_attr($trim_term->name) ?></span>
                                <?php
                            }
                            ?>
                        </div>
        ...
        static function th_product_taxonomy_metabox_edit($tag){
            ...
            switch ($taxonomy){
                ...
                case 'tax_product_model':
                    ?>
                    <tr class="form-field">
                        <th><label><?php esc_html_e('Related Make','autopart'); ?></label></th>
                        <td>
                            <div class="wrap-metabox">
                                <?php
                                $make_terms = get_terms( 'tax_product_make', array(
                                ...
                                $cf_terms = get_term_meta($tag->term_id, 'related_make_tax', false);
                                $terms_selected = $cf_terms[0];
                                foreach ($make_terms as $key => $make_term){
                                    $selected_attr = "";
                                    if(is_array($terms_selected)){
                                        foreach ($terms_selected as $key => $selected) {
                                            if($selected == $make_term->slug){
                                                $selected_attr = "checked";
                                            }
                                        }
                                    }
                                    ?>
                                    <input type="checkbox"  name="related_make_tax[]" value="<?php echo esc_attr($make_term->slug) ?>"  <?php echo esc_attr($selected_attr) ?> /> <span><?php echo esc_attr($make_term->name) ?></span>
                                    <?php
                                }
                                ?>
                            </div>
                    ...
                    <tr class="form-field">
                        <th><label><?php esc_html_e('Related Year','autopart'); ?></label></th>
                        <td>
                            <div class="wrap-metabox">
                                <?php
                                $year_terms = get_terms( 'tax_product_year', array(
                                ...
                                $cf_terms = get_term_meta($tag->term_id, 'related_year_tax', false);
                                $terms_selected = $cf_terms[0];
                                foreach ($year_terms as $key => $year_term){
                                    $selected_attr = "";
                                    if(is_array($terms_selected)){
                                        foreach ($terms_selected as $key => $selected) {
                                            if($selected == $year_term->slug){
                                                $selected_attr = "checked";
                                            }
                                        }
                                    }
                                    ?>
                                    <input type="checkbox"  name="related_year_tax[]" value="<?php echo esc_attr($year_term->slug) ?>"  <?php echo esc_attr($selected_attr) ?> /> <span><?php echo esc_attr($year_term->name) ?></span>
                                    <?php
                                }
                                ?>
                            </div>
                    ...
                    <tr class="form-field">
                        <th><label><?php esc_html_e('Related Trim','autopart'); ?></label></th>
                        <td>
                            <div class="wrap-metabox">
                                <?php
                                $trim_terms = get_terms( 'tax_product_trim', array(
                                ...
                                $cf_terms = get_term_meta($tag->term_id, 'related_trim_tax', false);
                                $terms_selected = $cf_terms[0];
                                foreach ($trim_terms as $key => $trim_term){
                                    $selected_attr = "";
                                    if(is_array($terms_selected)){
                                        foreach ($terms_selected as $key => $selected) {
                                            if($selected == $trim_term->slug){
                                                $selected_attr = "checked";
                                            }
                                        }
                                    }
                                    ?>
                                    <input type="checkbox"  name="related_trim_tax[]" value="<?php echo esc_attr($trim_term->slug) ?>"  <?php echo esc_attr($selected_attr) ?> /> <span><?php echo esc_attr($trim_term->name) ?></span>
                                    <?php
                                }
                                ?>
                            </div>
        ...
        static function th_product_save_taxonomy_metadata($term_id){
           // var_dump($_POST['related_make_tax']);
            $tax_names = array('make','model','year','trim','vin');
            foreach ($tax_names as $key => $value){
                if (isset($_POST['related_'.$value.'_tax'])) update_term_meta( $term_id, 'related_'.$value.'_tax', $_POST['related_'.$value.'_tax']);
                if (isset($_POST['related_'.$value.'_tax'])) update_term_meta( $term_id, 'related_'.$value.'_tax', $_POST['related_'.$value.'_tax']);
            }
        }
        ...
        if($th_woo_widgets != 'true' && isset($active_widgets['woocommerce-sidebar']) && empty($active_widgets['woocommerce-sidebar'])){
            ...
            $widgets = array(
                'woocommerce_product_categories' => array(
                    'title' => esc_html__('Product categories','autopart'),
                    'orderby' => 'name',
                    'dropdown' => 0,
                    'count' => 0,
                    'hierarchical' => 1,
                ...
                'woocommerce_price_filter' => array(
                    'title' => esc_html__('Filter by price','autopart'),
                    ),
                'woocommerce_products' => array(
                    'title' => esc_html__('Products','autopart'),
                    'number' =>  5,
                    'show' => '',
                    'orderby' => 'date',
                    'order' => 'desc',
