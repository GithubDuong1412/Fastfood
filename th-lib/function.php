...
if(!function_exists('th_list_post_type')){
    function th_list_post_type($post_type = 'page',$add_empty = false,$convert = false){
        ...
        $page_list = array();
        if($add_empty){
            if(!$convert) $page_list[''] = esc_html__('-- Choose One --','autopart');
            else{
                $page_list[] = array(
...
if ( !function_exists( 'th_get_google_link' ) ) {
    function th_get_google_link() {
        ...
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'autopart' ) ) {
        ...
        <div id="loading" class="preload-loading preload-style-<?php echo esc_attr($preload_style)?>">
            <div id="loading-center">
                ...
                switch ($preload_style) {
                    ...
                    case 'style8':
                        ?>
                        <div id="loading-center-absolute<?php echo esc_attr($preload_style)?>" class="car-preload">
                            <div class="car">
                            ...
                            <div class="text">
                                <span><?php echo esc_html__("Loading","autopart")?></span><span class = "dots">...</span>
                            </div>
                        </div>
                        <?php
                        break;
                    ...
                    case 'custom-image':
                        ...
                        <div id="loading-center-absolute-image">
                            <img src="<?php echo esc_url($preload_img)?>" alt="<?php esc_attr_e("preload-image","autopart");?>"/>
                        </div>
                        <?php
                        break;
...
if(!function_exists('th_get_main_class')){
    function th_get_main_class(){
        ...
        if($sidebar_pos != 'no' && is_active_sidebar( $sidebar['id'])){
            $main_class = 'content-wrap content-sidebar-'.$sidebar_pos.' col-md-9 col-sm-8 col-xs-12';
            if($sidebar_style == 'sidebar-toggle') $main_class = 'content-wrap col-md-12 col-sm-12 col-xs-12 sidebar-toggle';
        }
                    ...
                    if(!empty($data)){
                        foreach ($data as $key => $value) {
                            switch ($value) {
                                case 'date':
                                    ?>
                                    <li class="meta-item"><i class="las la-calendar-check"></i><span class="silver"><?php echo get_the_date()?></span></li>
                                ...
                                case 'tags':
                                    ...
                                    if($tags):?>
                                        <li class="meta-item"><i class="la la-tags" aria-hidden="true"></i>
                                            ...
                                            <?php if($tags) echo apply_filters('th_output_content',$tags); else esc_html_e("No Tag",'autopart');?>
                                        </li>
                                        <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
                                    <?php endif;
                                    break;
                                ...
                                case 'comments':
                                    ...
                                    <li class="meta-item"><i aria-hidden="true" class="la la-comment"></i>
                                        ...
                                        <?php 
                                            if(get_comments_number() != 1) esc_html_e('Comments', 'autopart') ;
                                            else esc_html_e('Comment', 'autopart') ;
                                        ?>
                                        </a>
                                    </li>
                                    <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
                                ...
                                case 'views':
                                    ...
                                    <li class="meta-item"><i class="la la-eye"></i>
                                        ...
                                        if(th_get_post_view() != 1) echo esc_html__("Views",'autopart');
                                        else echo esc_html__("View",'autopart');
                                        ?>
                                        </span>
                                    </li>
                                    <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
...
if(!function_exists('th_get_post_list_style')){
    function th_get_post_list_style(){
        $list = apply_filters('th_post_list_item_style',array(
            ...
            'style2' => esc_html__('Post list 2','autopart'),
            'style3' => esc_html__('Post list 3','autopart'),
            'style4' => esc_html__('Post list 4','autopart'),
            'style5' => esc_html__('Post list 5','autopart'),
            ));        
        return $list;
    }
}
if(!function_exists('th_get_post_style')){
    function th_get_post_style($style = 'element'){
        $list = apply_filters('th_post_item_style',array(
            '' => esc_html__('Default','autopart'),
            'style2' => esc_html__('Post grid 2','autopart'),
            ));
...
if(!function_exists('th_get_product_list_style')){
    function th_get_product_list_style(){
        $list = apply_filters('th_product_list_item_style',array(
            ''          => esc_html__('Default','autopart'),
            'style2'    => esc_html__('Product list 2','autopart'),
            ));
        return $list;
    }
}
if(!function_exists('th_get_product_style')){
    function th_get_product_style($style = 'element'){
        $list = apply_filters('th_product_item_style',array(
            ''          => esc_html__('Default','autopart'),
            'style2'    => esc_html__('Product grid 2','autopart'),
            'style3'    => esc_html__('Product grid 3','autopart'),
            'style4'    => esc_html__('Product grid 4','autopart'),
            'style5'    => esc_html__('Product grid 5','autopart'),
            'style6'    => esc_html__('Product grid 6','autopart'),
            'style8'    => esc_html__('Product grid 8','autopart'),
            'style9'    => esc_html__('Product grid 9','autopart'),
            ));
        return $list;
    }
}
...
if(!function_exists('th_compare_url')){
    function th_compare_url($icon='',$id = false,$text = '',$class='silver'){
        $html = '';
        if(empty($text)) $text = esc_html__("Compare","autopart");
...
if(!function_exists('th_wishlist_url')){
    function th_wishlist_url($icon='',$text='',$class='silver'){
        $html = '';
        if(empty($text)) $text = esc_html__("Wishlist","autopart");
...
if(!function_exists('th_fill_css_background')){
    function th_fill_css_background($data, $imp=''){
        $string = '';
        if(!empty($data['background-color'])) $string .= 'background-color:'.$data['background-color'].$imp.';';
        if(!empty($data['background-repeat'])) $string .= 'background-repeat:'.$data['background-repeat'].$imp.';';
        if(!empty($data['background-attachment'])) $string .= 'background-attachment:'.$data['background-attachment'].$imp.';';
        if(!empty($data['background-position'])) $string .= 'background-position:'.$data['background-position'].$imp.';';
        if(!empty($data['background-size'])) $string .= 'background-size:'.$data['background-size'].$imp.';';
        if(!empty($data['background-image'])) $string .= 'background-image:url("'.$data['background-image'].'")'.$imp.';';
...
if(!function_exists('th_breadcrumb')){
    function th_breadcrumb($step = '') {
        global $post;
        if(is_home() && !is_front_page()) echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','autopart').'</a>'.$step.'<span>'.esc_html__('Blog','autopart').'</span>';
        else echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','autopart').'</a>';
        if (is_single()){
        ...
        elseif(is_search()) echo apply_filters('s7upf_output_content',$step)."<span>".esc_html__("Search Results for: ","autopart").get_search_query().'</span>';
        elseif(is_404()) echo apply_filters('s7upf_output_content',$step)."<span>".esc_html__("404 ","autopart")."</span>";
    }
}
if(!function_exists('th_fix_import_category')){
    function th_fix_import_category($taxonomy){
...
if(!function_exists('th_register_user')){
    function th_register_user(){
        ...
        $output = array();
        $output['status'] = 'error';
        $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your username or email has been exist. Please check again. ","autopart").'</div></div>';
        if(!empty($user_data)){
            if(!username_exists($user_data['user_login']) && !email_exists($user_data['user_email'])){                
                ...
                $output['message'] = '<div class="message login_info ms-info ms-default">'.esc_html__("Account registration has been successful","autopart").'</div>';
            }            
            if(username_exists($username)){
                $output['status'] = 'error';
                $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your username has been exist.","autopart").'</div></div>';
            }           
            if(username_exists($email)){
                $output['status'] = 'error';
                $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your email has been exist.","autopart").'</div></div>';
            }
        }
        echo ''.$output['message'];
        die();
...
if(!function_exists('th_get_elementor_settings')){
    function th_get_elementor_settings(){        
        $args = array("post_type" => "elementor_library", "s" => 'Default Kit');
        $query = get_posts( $args );
        if(isset($query[0])){
            $data = get_post_meta($query[0]->ID,'_elementor_page_settings');
        }
        wp_reset_postdata();
